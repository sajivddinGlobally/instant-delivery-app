...
class _PickupScreenState extends State<PickupScreen> {
  ...
  void _sendMessage() {
    ...
    socket.emit('send_message', {'message': message, 'deliveryId': widget.deliveryId});
    log('📤 Sent message: $message');
  ...
  Widget build(BuildContext context) {
    ScreenUtil.init(context);
    final driverName = '${widget.driver['firstName'] ?? ''} ${widget.driver['lastName'] ?? ''}'.trim();
    final driverPhone = widget.driver['phone']?.toString() ?? 'N/A';
    final pickupAddress = widget.pickup?['name'] ?? 'Unknown Pickup';
    final dropoffAddress = widget.dropoff?['name'] ?? 'Unknown Dropoff';
              ...
              return Container(
                ...
                child: ListView(
                  ...
                  children: [
                      ...
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Text(
                              "Car details not available", // Hardcoded; update if payload includes car info
                              style: GoogleFonts.inter(
                                fontSize: 13.sp,
                                color: Colors.grey[700],
                                letterSpacing: -1,
                            ...
                            const Row(
                              children: [
                                ...
                                Text(
                                  "4.9", // TODO: Make dynamic if rating in payload
                                  style: TextStyle(
                                    fontSize: 13,
                                    color: Colors.black,
                                  ),
                    ...
                    /// PAYMENT DETAILS (Hardcoded; update if fare in payload)
                    const Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        const Text(
                          "₹120", // TODO: Make dynamic if fare in payload
                          style: TextStyle(
                            fontWeight: FontWeight.w600,
                          ),
                        ),
                        ...
                        Expanded(
                          child: ElevatedButton(
                            onPressed: () {
                              cancelOrderApi(
                                  CancelOrderModel(
                                      txId:widget.deliveryId,
                                      cancellationReason: 'jsdvfj')
...
class _PickupScreenState extends State<PickupScreen> {
  ...
  void _showCancelBottomSheet() {
    ...
    showModalBottomSheet(
                              ...
                              onChanged: (int? value) {
                                setModalState(() {
                                  ...
                                  if (value != reasons.length - 1) {
                                    _otherController.clear(); // Clear other field if not selecting Other
                                  }
                                });
                              },
      ...
      // Dispose controller after sheet closes
      _otherController.dispose();
    });
  }
  @override
...
//       initialChildSize: 0.6,
//       minChildSize: 0.4,
//       maxChildSize: 0.9, // Increased slightly to accommodate TextField
//       builder: (context, scrollController) {
//         return Container(
//           padding: EdgeInsets.all(20.w),
//           decoration: const BoxDecoration(
...
//                           _isLoading = true;
//                         });
//                         widget.onCancel(); // Close sheet first
...
//                         );
//                         if (mounted) {
//                           Navigator.push(context, MaterialPageRoute(builder: (context)=>SelectTripScreen()));
...
//   }
//
//   Future<void> cancelOrderApiStatic(CancelOrderModel body) async {
//     try {
//       final service = APIStateNetwork(callPrettyDio());
//       final response = await service.deliveryCancelledByUser(body);
//
