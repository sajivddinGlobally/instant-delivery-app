...
class NewScreen extends StatefulWidget {
  String driverId; // Assuming this is the deliveryId for the API call
  NewScreen({
...
class _NewScreenState extends State<NewScreen> {
  ...
  Future<void> _fetchDeliveryData() async {
    try {
      ...
      // Call the API with driverId as deliveryId
      final response = await service.getDeliveryById(widget.driverId);
    ...
    return Scaffold(
      body: Column(
        children: [
          ...
          //   // Display data from API response (adjust based on your model structure)
          //     Expanded(
          //       child: ListView(
          //         children: [
          //           Text('Delivery ID: ${widget.driverId}'),
          //           // Example: Add more fields from deliveryData, e.g.,
          //           // Text('Status: ${deliveryData?.status ?? 'N/A'}'),
          //           // Text('Driver: ${deliveryData?.driverName ?? 'N/A'}'),
          //         ],
          //       ),
          //     )
          //   else
...
//     return Scaffold(
//       appBar: AppBar(
//         title: Text('Delivery Details - ${widget.driverId}'),
...
//             )
//           else if (deliveryData != null && deliveryData!.data != null)
//               Expanded(
//                 child: ListView(
//                   padding: const EdgeInsets.all(16.0),
//                   children: [
//                     _buildSectionTitle('Delivery Info'),
//                     _buildInfoRow('Delivery ID', deliveryData!.data!.id ?? 'N/A'),
//                     _buildInfoRow('Status', deliveryData!.data!.status ?? 'N/A'),
//                     _buildInfoRow('Payment Method', deliveryData!.data!.paymentMethod ?? 'N/A'),
//                     _buildInfoRow('Transaction ID', deliveryData!.data!.txId ?? 'N/A'),
//                     _buildInfoRow('Created At', _formatDateTime(deliveryData!.data!.createdAt?.toString() ?? '')),
//                     const SizedBox(height: 16),
//
//                     _buildSectionTitle('Pickup Location'),
//                     _buildLocationCard(deliveryData!.data!.pickup),
//                     const SizedBox(height: 16),
//
//                     _buildSectionTitle('Dropoff Location'),
//                     _buildLocationCard(deliveryData!.data!.dropoff),
//                     const SizedBox(height: 16),
//
//                     _buildSectionTitle('Package Details'),
//                     _buildInfoRow('Fragile', (deliveryData!.data!.packageDetails?.fragile ?? false) ? 'Yes' : 'No'),
//                     const SizedBox(height: 16),
//
//                     _buildSectionTitle('Customer Details'),
//                     _buildInfoRow('Name', '${deliveryData!.data!.customer?.firstName ?? ''} ${deliveryData!.data!.customer?.lastName ?? ''}'),
//                     _buildInfoRow('Email', deliveryData!.data!.customer?.email ?? 'N/A'),
//                     _buildInfoRow('Phone', deliveryData!.data!.customer?.phone ?? 'N/A'),
//                     _buildInfoRow('Current Lat/Lng', _formatCoordinates(deliveryData!.data!.customer?.currentLocation?.coordinates)),
//                     _buildInfoRow('Completed Orders', deliveryData!.data!.customer?.completedOrderCount?.toString() ?? '0'),
//                     const SizedBox(height: 16),
//
//                     _buildSectionTitle('Delivery Boy'),
//                     _buildInfoRow('Driver ID', deliveryData!.data!.deliveryBoy ?? 'N/A'),
//                   ],
//                 ),
//               )
//             else
...
//   }
//
//   Widget _buildLocationCard(Dropoff? location) {
//     if (location == null) return const Text('No location data');
//     return Card(
//       child: Padding(
...
//           crossAxisAlignment: CrossAxisAlignment.start,
//           children: [
//             Text(location.name ?? 'N/A', style: const TextStyle(fontWeight: FontWeight.bold)),
//             Text('Lat: ${location.lat?.toStringAsFixed(4) ?? 'N/A'}'),
//             Text('Long: ${location.long?.toStringAsFixed(4) ?? 'N/A'}'),
//           ],
//         ),
//       ),
//     );
...
//   }
//
//   String _formatCoordinates(List<double>? coords) {
//     if (coords == null || coords.isEmpty) return 'N/A';
//     return 'Lat: ${coords[1].toStringAsFixed(4)}, Lng: ${coords[0].toStringAsFixed(4)}'; // GeoJSON: [lng, lat]
//   }
// }
...
// import 'package:delivery_mvp_app/data/Model/GetDeliveryByIdResModel.dart'; // Add this import
...
//
// class NewScreen extends StatefulWidget {
//   final String deliveryId; // Keep only deliveryId
//   const NewScreen({
//     super.key,
//     required this.deliveryId, // Make required
//   });
//
//   @override
//   State<NewScreen> createState() => _NewScreenState();
...
//     _getCurrentLocation();
//     _initializeSocket();
//     _fetchDeliveryData(); // Fetch API data
//   }
//
//   Future<void> _fetchDeliveryData() async {
//     try {
...
//           txId: deliveryData?.data?.txId ?? '', // Use fetched txId
//           onCancel: () {
...
//     final driverName = data.deliveryBoy ?? 'Unknown Driver'; // Use deliveryBoy ID as name (fetch full driver if API available)
//     final driverPhone = 'N/A'; // Not in API; fetch driver profile if needed
//     final averageRating = '0'; // Not in API; fetch if needed
...
//     final otp = 'N/A'; // From notification; pass separately if needed
//     final amount = 'N/A'; // Not in API; calculate or fetch from booking
//     final status = data.status?.toString() ?? 'N/A';
//     final txId = data.txId?.toString() ?? 'N/A';
//
//     final vehicleTypeName = 'N/A'; // Not in API
//     final baseFare = '0';
//     final perKmRate = '0';
//     final vehicalImage = "https://media.istockphoto.com/id/1394758946/vector/no-image-raster-symbol-missing-available-icon-no-gallery-for-this-moment-placeholder.jpg?s=170667a&w=0&k=20&c=HMFTtins81JmJWSrFbjs-xNL_W0KXonnGwCWJo5IPp0="; // Default
//
//     return Scaffold(
...
//                         CircleAvatar(
//                           radius: 25,
//                           backgroundImage: NetworkImage(vehicalImage),
//                         ),
//                         SizedBox(width: 12.w),
//                         Column(
//                           crossAxisAlignment: CrossAxisAlignment.start,
...
//                             onPressed: _showCancelBottomSheet, // Updated to show bottom sheet
//
...
//   final String txId; // Changed to txId
